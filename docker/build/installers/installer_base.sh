...
export RCFILES_DIR="/opt/myCXX/rcfiles"
...
export LOCAL_CACHE_ADDR="/opt/myCXX/installers/cache"
...
function download_if_not_cached {
    ...
    if [[ -f "${LOCAL_CACHE_ADDR}/${pkg_name}" ]]; then
        local local_addr="${LOCAL_CACHE_ADDR}/${pkg_name}"
        if _checksum_check_pass "${pkg_name}" "${expected_cs}"; then
            ok "Successfully found ${pkg_name} from ${LOCAL_CACHE_ADDR}," \
                "will use it."
            return
        else
            warning "Found ${pkg_name} in local cache, but checksum mismatch."
            rm -f "${pkg_name}"
        fi
    fi
